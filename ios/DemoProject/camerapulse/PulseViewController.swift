...
public protocol PulseViewDelegate {
  func onHeartRateReceved(_ heartRate:Any)
}


public class PulseViewController: UIViewController {
  ...
  private var capturedPulse: [Int] = []
  
  var pulseViewDelegate:PulseViewDelegate!
  
  public init(pulseViewDelegate:PulseViewDelegate) {
    super.init(nibName: "PulseViewController", bundle: Bundle(for: PulseViewController.self))
    self.pulseViewDelegate = pulseViewDelegate
    self.capturedPulse = []
  }
      ...
      self.timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true, block: { [weak self] (timer) in
        ...
        if pulse == -60 {
          ...
          UIView.animate(withDuration: 0.2, animations: {
            ...
            let pulseValue = lroundf(pulse)
            self.pulseLabel.text = "\(pulseValue) BPM"
            self.capturedPulse.append(pulseValue)
            self.checkAndReturnTheData()
          }
        }
      })
    }
...
extension PulseViewController {
  ...
  private func checkAndReturnTheData(){
    if self.capturedPulse.count > 4 {
      DispatchQueue.main.async {
        self.dismiss(animated: true) {
          self.pulseViewDelegate?.onHeartRateReceved([ "heartRate":self.capturedPulse.average, "status": true, "error": "Captured Heart rate From Camera" ])
        }
      }
    }
  }
